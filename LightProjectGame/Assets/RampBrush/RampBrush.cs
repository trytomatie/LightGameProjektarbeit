...
//			Gizmos.DrawRay(endClipPlaneOrig, endClipPlane.normal * 50f);
	...
	public void rampBrush() {
		...
		try { 
			...
			//don't use InverseTransformPoint because the terrain ignores rotation and scale fields.
			Vector3 localBeginPosition = beginRamp - transform.position; //transform.InverseTransformPoint(beginRamp);
			Vector3 localEndPosition = endRamp - transform.position;     //transform.InverseTransformPoint(endRamp);
